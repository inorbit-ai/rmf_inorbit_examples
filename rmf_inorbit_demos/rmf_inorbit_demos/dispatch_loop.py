...
#
# This file has been copied and adapted from open_rmf/rmf_demos package:
...
"""Generates and send a request to /submit_task service of a loop action task"""
...
class TaskRequester:
    ...
    def __init__(self, argv=sys.argv):
        ...
        parser.add_argument('-n', '--loop_num',
                            help='Number of loops to perform, default: 1',
                            type=int, default=1)
    ...
    def main(self, service_timeout=3.0, response_timeout=3.0, task_delay_timeout=1.0):
        if not self.submit_task_srv.wait_for_service(timeout_sec=service_timeout):
            self.node.get_logger().error(
                f'Could not reach /submit_task srv after {service_timeout} seconds')
            return
